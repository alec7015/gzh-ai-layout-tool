...
- 复制到公众号：列表/表格重塑、外链图 Base64、`text/html` + `execCommand` 兜底：`src/domain/clipboard.ts`、`src/domain/wechatCopyPipeline.ts`
- 草稿列表 / 版本历史 / 设置 / 自定义版式保存：`src/domain/draftLibrary.ts`、`src/domain/aiSettings.ts`、`src/domain/customStyles.ts`
- 图片拖拽 / 粘贴 / Markdown 图片块 / 多图版式：`src/domain/imageAssets.ts`、`src/domain/imageGrid.ts`
- 魔法粘贴富文本净化：`src/domain/magicPaste.ts`
- 表格最小解析与微信兼容渲染：`src/domain/draftStore.ts`、`src/domain/wechatRenderer.ts`
...
当前覆盖 18 个测试文件，包含草稿列表、版本历史、Tiptap AST 往返、多图、魔法粘贴、微信复制流水线、渲染、复制和 AI 兜底。
